...
import { MapPin, Globe, Calendar, BookOpen, ShieldCheck, ExternalLink, CheckCircle2 } from 'lucide-react';
...
export function UniversityDetail({ university, isOpen, onClose }: UniversityDetailProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] p-0 overflow-hidden bg-white dark:bg-slate-950 border-none shadow-2xl">
        <div className="relative h-32 bg-gradient-to-r from-slate-900 to-slate-800 flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 text-center p-6 w-full">
            <h2 className="text-3xl font-bold text-white font-heading tracking-tight">{university.name}</h2>
            <div className="flex justify-center items-center gap-2 mt-2 text-slate-300">
              <MapPin className="w-4 h-4" />
              <span className="text-sm font-medium">{university.city}, {university.region}</span>
            </div>
          </div>
        </div>
        ...
        <ScrollArea className="max-h-[calc(90vh-8rem)]">
          <div className="p-6 space-y-8">
            <div className="flex flex-wrap gap-3">
              <Badge className={`px-3 py-1 text-sm ${university.type === 'Public' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-amber-600 hover:bg-amber-700'}`}>
                {university.type} University
              </Badge>
              {university.englishPrograms && (
                <Badge variant="outline" className="px-3 py-1 text-sm border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20">
                  <CheckCircle2 className="w-3 h-3 mr-1" /> English Programs Available
                </Badge>
              )}
              <Badge variant="outline" className="px-3 py-1 text-sm border-slate-300 dark:border-slate-700">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {university.domains.map((domain) => (
                      <Badge key={domain} variant="secondary" className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                        {domain}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {university.languages.map(lang => (
                        <span key={lang} className="text-sm font-medium text-foreground bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">
                          {lang}
                        </span>
                      ))}
                    </div>
